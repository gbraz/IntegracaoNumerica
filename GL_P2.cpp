...
double GL_P2::integrar(){
    ...
    if (m_particao >= 1)
    {
        ...
        for (int i = 0; i < m_particao; i++)
        {
            ...
            integral += GL_P2_integracao(m_pIntegrando, linf, lsup);
        }
...
double GL_P2::GL_P2_integracao(Funcao * f, double a, double b)
{
    return (b-a)/2 * ( m_pIntegrando->f(((b-a)*(-0.57735) + (a+b))/2)
                      + m_pIntegrando->f(((b-a)*(0.57735) + (a+b))/2));
}
